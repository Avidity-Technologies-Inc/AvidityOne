...
if name == 'hostname': print('avidityhelpdesk')
    ...
    web = root / 'app/apps/web/.next'
    web.mkdir(parents=True, exist_ok=True)
    (web / 'BUILD_ID').write_text('new web')
...
class DirectEmailDeploymentTests(unittest.TestCase):
    def run_deployment(self, failure='', previous='daedfde4a616d1c2804bb017ee54021fcb4a16ff'):
        with tempfile.TemporaryDirectory(prefix='avidity-deploy-test-') as directory:
            ...
            (web / 'BUILD_ID').write_text('old web')
            ...
            for name in ('hostname', 'runuser', 'git', 'systemctl', 'node', 'curl', 'sleep', 'npm'):
                ...
                path.chmod(0o755)
            script = (ROOT / 'scripts/deploy-ticket-email-direct.sh').read_text()
            script = script.replace('app=/opt/avidity/app', f'app={app}')
            script = script.replace('/opt/avidity/ticket-email-direct-backup.', f'{root}/backup.')
            ...
            self.assertEqual((web / 'BUILD_ID').read_text(), 'old web' if failure else 'new web', result.stdout + result.stderr)
