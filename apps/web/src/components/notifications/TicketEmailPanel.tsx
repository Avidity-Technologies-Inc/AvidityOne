...
const options: Array<{ key: keyof Pick<Policy, "enabled" | "repliesEnabled" | "closeEnabled" | "includeHistory" | "includeAttachments" | "includeInternal" | "includeTeams" | "includeGroups" | "includeWatchers">; label: string; help: string }> = [
  ...
  { key: "repliesEnabled", label: "Allow replies from email", help: "Authorized replies are recorded and sent directly from the specialist's registered email." },
  { key: "closeEnabled", label: "Allow [Closed] command", help: "First line of new text only. Closes the ticket with the reply, using current assignment and close permissions. No confirmation step." }
];
export function TicketEmailPanel({ own = false }: { own?: boolean }) {
  ...
  return <section className="ticket-email-panel settings-section" aria-label="Ticket email operations">
    <div className="section-heading"><div><h3>Ticket email operations</h3><p className="muted">Complete conversation copies, direct email replies and delivery tracking.</p></div><button className="button secondary compact-button" disabled={busy || dirty} onClick={() => void load()}>Refresh email status</button></div>
    {error ? <p role="alert">{error}</p> : null}{status ? <p role="status">{status}</p> : null}
    {draft ? <>
      <p className="notification-warning-banner">{data?.policy.enabled ? "Full ticket email is enabled." : "Full ticket email is off; existing notification behavior is preserved."} {data?.policy.repliesEnabled && data?.policy.enabled ? "Authorized email replies and [Closed] commands execute directly. No confirmation is required." : "Respond using the platform until email replies are enabled."} External specialists are not automatically subscribed or authorized to execute commands.</p>
      ...
      <details><summary>Recent deliveries and email actions</summary><p className="muted">Last 50 records in each list. Accepted means the provider accepted the email, not proof of inbox receipt. Simulated means no real email was sent. Interrupted actions must be reviewed in the ticket before resubmission. Legacy pending confirmations are not executed; send a new reply to use direct processing.</p>
        ...
        <div className="ticket-email-history"><table><caption>Email actions</caption><thead><tr><th>Operation</th><th>Status</th><th>Created</th></tr></thead><tbody>{data?.actions.map((row) => <tr key={row.id}><td><a href={`/tickets/${row.ticketId}`}>{row.ticketNumber}</a>{!own ? <small>{row.userName} · {row.userEmail}</small> : null}<br/>{row.mode === "INTERNAL" ? "Internal note" : "Public reply"}{row.closeTicket ? " + close" : ""}{row.error ? <small>{row.error}</small> : null}</td><td>{row.status === "AWAITING_CONFIRMATION" ? "Legacy pending — send a new reply" : row.status.replaceAll("_", " ")}</td><td>{new Date(row.createdAt).toLocaleString()}</td></tr>)}</tbody></table>{!data?.actions.length ? <p className="muted">No email actions yet.</p> : null}</div>
      </details>
