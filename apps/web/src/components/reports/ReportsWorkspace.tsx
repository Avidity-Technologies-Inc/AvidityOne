...
export function ReportsWorkspace() {
  ...
  const rowSortKey = applied.sortBy || (isProject ? "" : "createdAt");
  const rowSortColumn = REPORT_COLUMNS[kind].find((column) => column.key === rowSortKey);
  const exportedSortColumn = columns.some((column) => column.key === rowSortKey);
  const rowSortDescription = rowSortColumn ? `${rowSortColumn.label} · ${applied.sortDirection === "asc" ? "Ascending" : "Descending"}` : "Delivery priority (default)";
