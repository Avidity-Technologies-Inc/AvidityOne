...
import { REPORT_COLUMNS, REPORT_DEFAULT_COLUMNS, REPORT_PERIODS, ReportColumn, ReportKind } from "@avidity/shared";
...
const initialPresentation = (kind: ReportKind): Presentation => ({ columns: REPORT_DEFAULT_COLUMNS[kind].join(","), sections: "summary,charts,detail", title: "", paper: "LETTER", orientation: "landscape", scope: "all" });
...
function Breakdowns({ report }: { report: Report }) {
  return <div className="report-breakdowns">{Object.entries(report).filter(([key, value]) => key.startsWith("by") && Array.isArray(value)).map(([key, value]) => {
...
export function ReportsWorkspace() {
  ...
  async function save(update: boolean) {
    ...
    await action(async () => {
      const filters: Filters = { ...(isProject ? {} : applied), ...presentation }; if (presentation.scope !== "page") delete filters.page;
  ...
  function exportQuery() {
    const frozen = { ...applied, ...presentation, sections: format === "csv" ? "detail" : presentation.sections, format, timeZone: zone };
    ...
    if (isProject) return params(Object.fromEntries(Object.entries(frozen).filter(([key]) => ["columns", "sections", "title", "paper", "orientation", "scope", "timeZone", "format"].includes(key))));
  ...
  function sort(key: string) { const next = { ...applied, sortBy: key, sortDirection: applied.sortBy === key && applied.sortDirection === "asc" ? "desc" : "asc", page: "1" }; setApplied(next); setDraft(next); }
  const sortable = isTicket ? ["ticketNumber", "subject", "clientName", "status", "priority", "source", "assignedTo", "team", "createdAt", "updatedAt", "closedAt", "resolvedAt"] : ["trackingNumber", "eventName", "eventDate", "status", "priority", "createdAt", "updatedAt", "clientName"];
  ...
  return <div className="reports-workspace reports-v2">
    ...
    {error ? <div role="alert" className="error-banner">{error}</div> : null}{notice ? <div role="status" className="report-notice"><Check size={16} />{notice}</div> : null}
    {tab === "overview" ? <>
      {!isProject ? <section className="panel report-filter-panel"><div className="report-section-heading"><div><h2>Report criteria</h2><p className="muted">Choose the records to include. Timezone defaults to the organization settings.</p></div><button className="button secondary" aria-expanded={advanced} onClick={() => setAdvanced(!advanced)}><Filter size={15} />{advanced ? "Fewer filters" : "More filters"}</button></div><div className="report-form-grid">
      ...
      </div><div className="report-filter-footer"><span className="muted">{dirty ? "Filters changed. Apply them to refresh the results." : "Filters applied."}</span><button className="button secondary" onClick={reset}>Reset</button><button className="button" disabled={loading || busy} onClick={apply}>Apply filters</button></div></section> : null}
      ...
      {report ? <div aria-busy={loading} className={fresh ? "" : "report-stale"}>
        ...
        <details className="report-criteria"><summary>Applied criteria</summary><ul>{activeCriteria.map((text) => <li key={text}>{text}</li>)}</ul></details>
        <div className="report-metrics">{Object.entries(report.summary).filter(([key, value]) => key !== "estimatedTotal" || value !== null).map(([key, value]) => <div className="panel report-metric" key={key}><span>{label(key)}</span><strong>{value === null ? "Not configured" : key === "estimatedTotal" && report.filters?.currency ? new Intl.NumberFormat(undefined, { style: "currency", currency: report.filters.currency }).format(value) : value.toLocaleString()}</strong><small>{key === "estimatedTotal" ? "Manual estimate; not revenue" : key === "totalTickets" || key === "totalRequests" ? "All matching records" : key === "unassignedTickets" ? "No technician or team" : key === "completedProjects" ? "Completed projects, outside active detail" : "Within the matching records"}</small></div>)}</div>
        <Activity report={report} /><Breakdowns report={report} />
        <section className="panel report-detail"><div className="report-section-heading"><div><h2>Report detail</h2><p className="muted">{isProject ? `${total} active projects` : `${total ? ((report.page ?? 1) - 1) * (report.pageSize ?? 25) + 1 : 0}–${Math.min(total, (report.page ?? 1) * (report.pageSize ?? 25))} of ${total} records`}. Column selection also controls the export. Files include active regular and inline attachments.</p></div><button className="button secondary" onClick={() => setExportOpen(true)}>Choose columns</button></div><div className="report-table-scroll"><table><thead><tr>{columns.map((col) => <th key={col.key} style={{ minWidth: (col.width ?? 20) * 7 }}>{!isProject && sortable.includes(col.key) ? <button disabled={!fresh || dirty} onClick={() => sort(col.key)}>{col.label}{applied.sortBy === col.key ? applied.sortDirection === "asc" ? <ArrowUp size={12} /> : <ArrowDown size={12} /> : null}</button> : col.label}</th>)}</tr></thead><tbody>{report.detail.length ? report.detail.map((row, index) => <tr key={String(row.id ?? row.projectId ?? index)}>{columns.map((col, c) => <td key={col.key} style={{ minWidth: (col.width ?? 20) * 7 }}>{c === 0 && ((isTicket && grants.includes("tickets.view")) || (!isTicket && !isProject && grants.includes("event_services.view"))) ? <a href={isTicket ? `/tickets/${row.id}` : `/event-services/${encodeURIComponent(String(row.trackingNumber))}`}>{cellValue(row, col, zone, report.filters?.currency)}</a> : cellValue(row, col, zone, report.filters?.currency)}</td>)}</tr>) : <tr><td colSpan={columns.length}>No matching records. Adjust the filters to broaden the report.</td></tr>}</tbody></table></div>{!isProject ? <div className="report-pagination"><label>Rows<select value={applied.pageSize} disabled={!fresh || dirty} onChange={(e) => { const next = { ...applied, pageSize: e.target.value, page: "1" }; setApplied(next); setDraft(next); }}>{[25, 50, 100].map((n) => <option key={n}>{n}</option>)}</select></label><span>Page {report.page} of {report.totalPages}</span><button className="button secondary" disabled={!fresh || dirty || report.page === 1} onClick={() => page((report.page ?? 1) - 1)}>Previous</button><button className="button secondary" disabled={!fresh || dirty || report.page === report.totalPages} onClick={() => page((report.page ?? 1) + 1)}>Next</button></div> : null}</section>
      </div> : null}
    </> : null}
