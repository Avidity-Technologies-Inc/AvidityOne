#!/usr/bin/env bash
# Native production deployment for the ticket operational email release.
set -Eeuo pipefail
umask 077
app=/opt/avidity/app
base=d8df408afee3208f2e008fce257621e093f8b6a5
release=${1:-}
...
[[ $(as_app git diff --name-only "$base" "$release" -- prisma/migrations) == prisma/migrations/20260921180000_ticket_email_operations/migration.sql ]] || { echo 'Unexpected migration scope; stop.'; exit 1; }
...
backup=$(mktemp -d /opt/avidity/ticket-email-backup.XXXXXXXX)
...
recover() {
  ...
  if [[ $stopped == 1 ]]; then
    ...
    echo 'Previous runtime restored. The additive email tables, capture triggers and message column are retained. Git remains at the attempted revision; verify health before retrying.'
  fi
  exit "$code"
}
trap recover ERR
...
echo 'Next: verify Settings > Notifications and Profile > Notifications. Operational email starts disabled. Verify mailbox permissions and run a controlled pilot before enabling it for staff. No test email has been sent by this script.'
