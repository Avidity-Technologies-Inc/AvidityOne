#!/usr/bin/env bash
# API-only deployment for ticket email threading; no schema, dependency or permission changes.
set -Eeuo pipefail
umask 077
app=/opt/avidity/app
base=367354e16b6d47ce118fb6ee4bd715399794cca4
release=${1:-}
...
backup=$(mktemp -d /opt/avidity/ticket-email-threading-backup.XXXXXXXX)
...
echo 'Threaded customer replies are active. Existing notification settings and Microsoft permissions are retained. Historical messages are not replayed or tickets automatically merged.'
