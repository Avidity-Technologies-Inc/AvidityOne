#!/usr/bin/env bash
# Native production deployment for the compact report controls release.
set -Eeuo pipefail
umask 077
app=/opt/avidity/app
base=6189269fe34ee6916f572060e0cd440f95a9f79a
release=${1:-}
...
[[ -z $(as_app git diff --name-only "$base" "$release" -- prisma) ]] || { echo 'Unexpected database change; stop.'; exit 1; }
...
backup=$(mktemp -d /opt/avidity/report-controls-backup.XXXXXXXX)
...
recover() {
  ...
  if [[ $stopped == 1 ]]; then
    ...
    echo 'Previous runtime restored. No database migration was applied. Git remains at the attempted revision; verify health before retrying.'
  fi
  exit "$code"
}
trap recover ERR
