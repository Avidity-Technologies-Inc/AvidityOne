#!/usr/bin/env bash
# Web-only deployment for the September 18 ticket composer fix.
set -Eeuo pipefail
umask 077
app=/opt/avidity/app
release=${1:-}
...
case "$previous" in
  409a48b0138b313744286e39c6395e5226e66023|43a5222eb4d6bb3681876f8d3bc5c1fc06be8332|"$release") ;;
...
[[ -z $(as_app git diff --name-only "$previous" "$release" -- apps/api packages prisma package.json package-lock.json apps/web/package.json apps/web/next.config.mjs apps/web/public) ]] || { echo 'Changes exceed a web-only deployment.'; exit 1; }
...
backup=$(mktemp -d /opt/avidity/composer-backup.XXXXXXXX)
