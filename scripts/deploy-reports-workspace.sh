#!/usr/bin/env bash
# Native production deployment for the reporting workspace and export release.
set -Eeuo pipefail
umask 077
app=/opt/avidity/app
base=52896197e99b1dd3b5158304a706d699a6c6dafc
release=${1:-}
...
[[ $(as_app git diff --name-only "$base" "$release" -- prisma/migrations) == prisma/migrations/20260921120000_report_schedule_timing/migration.sql ]] || { echo 'Unexpected migration scope; stop.'; exit 1; }
...
backup=$(mktemp -d /opt/avidity/reports-backup.XXXXXXXX)
...
recover() {
  ...
  if [[ $stopped == 1 ]]; then
    ...
    echo 'Previous runtime restored. The additive database column is retained. Git remains at the attempted revision; verify health before retrying.'
  fi
  exit "$code"
}
trap recover ERR
...
echo 'Next: verify Reports, one PDF and Excel export, and saved reports. No test email has been sent by this script.'
